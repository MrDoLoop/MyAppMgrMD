...
public class AppUpdateStaticReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (handleEvent) {
          //覆盖安装会先收到removed，然后在是added
            String PkgName = intent.getDataString().substring(8);
            // String packageName = intent.getData().getSchemeSpecificPart();
            String appName = Utilities.pkgNameToAppName(context, PkgName);
            if (intent.getAction().equals(Intent.ACTION_PACKAGE_REMOVED)) {
                ...
                DaoUtils.destroy();
                Toast.makeText(context, "MD卸载: " + PkgName, Toast.LENGTH_SHORT).show();
                
            } else if (intent.getAction().equals(Intent.ACTION_PACKAGE_ADDED)) {
                Log.i("ttt", "AppUpdateStaticReceiver: added-- " + appName + " -- " + PkgName);
                AppInfo app = Utilities.buildAppInfoEntry(context, PkgName);
                ...
                Toast.makeText(context, "MD新安装: " + appName, Toast.LENGTH_SHORT).show();
                ...
                DaoUtils.destroy();
                //EventBus.getDefault().post(new AppUpdateEvent(intent.getAction(),PkgName, app));
                Toast.makeText(context, "MD-APP改变了: " + appName, Toast.LENGTH_SHORT).show();
            }
        }
    }
