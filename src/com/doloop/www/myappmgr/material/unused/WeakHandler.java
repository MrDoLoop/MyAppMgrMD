 ...
 * <p/>
 * Original implementation of Handlers always keeps hard reference to handler in queue of execution. If you create
 * anonymous handler and post delayed message into it, it will keep all parent class for that time in memory even if it
 * could be cleaned.
 * <p/>
 * This implementation is trickier, it will keep WeakReferences to runnables and messages, and GC could collect them
 * once WeakHandler instance is not referenced any more
 * <p/>
 *
...
public class WeakHandler {
    ...
    /**
     * Constructor associates this handler with the {@link Looper} for the current thread and takes a callback interface
     * in which you can handle messages.
     *
     * If this thread does not have a looper, this handler won't be able to receive messages so an exception is thrown.
     *
    ...
    /**
     * Use the provided {@link Looper} instead of the default one and take a callback interface in which to handle
     * messages.
     *
     * @param looper The looper, must not be null.
     * @param callback The callback interface in which to handle messages, or null.
     ...
     * <var>uptimeMillis</var>. <b>The time-base is {@link android.os.SystemClock#uptimeMillis}.</b> The runnable will
     ...
     * received in callback, in the thread attached to this handler.
     *
     * @return Returns true if the message was successfully placed in to the message queue. Returns false on failure,
     *         usually because the looper processing the message queue is exiting.
     */
     ...
     * receive it in callback, in the thread attached to this handler.
     *
     * @return Returns true if the message was successfully placed in to the message queue. Returns false on failure,
     *         usually because the looper processing the message queue is exiting. Note that a result of true does not
     ...
     * <var>uptimeMillis</var>. <b>The time-base is {@link android.os.SystemClock#uptimeMillis}.</b> You will receive it
     * in callback, in the thread attached to this handler.
     *
     ...
     * You will receive it in callback, in the thread attached to this handler. <b>This method is only for use in very
     * special circumstances -- it can easily starve the message queue, cause ordering problems, or have other
    ...
    private WeakRunnable wrapRunnable(Runnable r) {
        ...
        return hardRef.wrapper =
                new WeakRunnable(new WeakReference<Runnable>(r), new WeakReference<ChainedRef>(hardRef));
    }
