...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
    ...
    private void enableFor(final View button, final View target, final int position) {
        // 变换图标
        if (lastOpenPosition == position) {//该位置是打开的
            if (button != null) {
                ViewParent vParent = button.getParent();
                View indView = ((ViewGroup) vParent).findViewById(R.id.indicator);
                if (ViewHelper.getRotation(indView) != -180.0f) {
                    ViewHelper.setRotation(indView, -180.0f);
                }
               ...
               /* ViewParent vParent = button.getParent();
                ...
                if (vParent == null) {
                    ...
                    if (vParent instanceof ViewGroup) {
                        indView = ((ViewGroup) vParent).findViewById(R.id.indicator);
                        if (ViewHelper.getRotation(indView) != -180.0f) {
                            ViewHelper.setRotation(indView, -180.0f);
                        }
                    }
            ...
            if (button != null) {
                ViewParent vParent = button.getParent();
                View indView = ((ViewGroup) vParent).findViewById(R.id.indicator);
                if (ViewHelper.getRotation(indView) != 0.0f) {
                    ViewHelper.setRotation(indView, 0.0f);
                }
    ...
    private void animateView(final View target, final int type) {
        ...
        ViewParent vParent = target.getParent();
        View indView = null;
        if (vParent == null) {
            Log.d("TEST", "this.getParent() is null");
        } else {
        ...
        final View indicatorView = indView;
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                if (type == ExpandCollapseAnimation.EXPAND) {
                    ...
                    if (indicatorView != null) { 
                       float curR = ViewHelper.getRotation(indicatorView);
                        if (curR != -180.0f) {
                            ObjectAnimator.ofFloat(indicatorView, "rotation", curR, -180.0f).setDuration(100).start();
                        }
                    }
                    ...
                    if (indicatorView != null) {
                        float curR = ViewHelper.getRotation(indicatorView);
                        if (curR != 0.0f) {
                            ObjectAnimator.ofFloat(indicatorView, "rotation", curR, 0.0f).setDuration(100).start();
                        }
                        
                        /*Animation curAnim = indicatorView.getAnimation();
                        
                        if(curAnim != null && !curAnim.hasEnded()){
                            curAnim.cancel();
                        }
    ...
    private void animateViewDurationZero(final View target, final int type) {
        Animation anim = new ExpandCollapseAnimation(target, type);
        anim.setDuration(0);
        ViewParent vParent = target.getParent();
        View indView = null;
     ...
     */
    public boolean collapseLastOpen(boolean playAnimation) {
        if (isAnyItemExpanded()) {
            ...
            if (lastOpen != null) {
                if (playAnimation) {
                    ...
                    animateViewDurationZero(lastOpen, ExpandCollapseAnimation.COLLAPSE);
                }
            }
            openItems.set(lastOpenPosition, false);
            lastOpenPosition = -1;
